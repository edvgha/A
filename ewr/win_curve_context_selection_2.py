...
def make_context_key(feat_str: pd.DataFrame,
    ...
    return feat_str[list(subset)].agg("|".join, axis=1)
...
def run_selection(df: pd.DataFrame, cfg: Config = Config(),
    ...
    base_suff = sufficiency_check(y, make_context_key(feat_str, base_subset),
                                  bid_levels, cfg, base_subset)
    base_cv = cv_log_loss_for_subset(df, feat_str, base_subset, cfg)
    ...
    while remaining and len(best_subset) < max_size:
        ...
        for feat in remaining:
            ...
            ctx = make_context_key(feat_str, candidate)

            # (1) sufficiency gate -- reject before paying for CV.
            suff = sufficiency_check(y, ctx, bid_levels, cfg, candidate)
            if not suff.passes:
            ...
            cv = cv_log_loss_for_subset(df, feat_str, candidate, cfg)
